...
const feed = JSON.parse(
  await readFile(resolve(websiteRoot, 'data/side-hustles.json'), 'utf8'),
);
...
for (const post of feed.posts) {
  ...
  invariant(!/——|--/.test(post.hook), `hook 含破折号：${post.topic_id}`);
  invariant(!post.hook.includes(','), `hook 含半角逗号：${post.topic_id}`);
  invariant(
  ...
  for (const problem of post.problems) {
    ...
    invariant(!/——|--|,/.test(problem), `问题含禁用标点：${problem}`);
  }
  invariant(
